...
def parse_birthday_document(doc):
    ...
    for para in doc.paragraphs:
        ...
        # Check if paragraph has any bold runs OR is a heading
        has_bold = any(run.bold for run in para.runs if run.bold is not None)
        is_heading = para.style.name.startswith('Heading') if para.style else False
        
        # Check if this is a date header (bold/heading with date format)
        if (has_bold or is_heading) and re.match(r'^[A-Z][a-z]+\s+\d+
...
def parse_service_document(doc):
    ...
    for para in doc.paragraphs:
        ...
        # Check if this is a year header (bold/heading text like "1 year" or "5 years")
        if has_bold or is_heading:
            year_match = re.match(r'^(\d+)\s+years?
...
def parse_birthday_document(doc):
    ...
    for para in doc.paragraphs:
        ...
        if has_bold and re.match(r'^[A-Z][a-z]+\s+\d+$', text):
            current_date = text
...
, text):
    ...
    # If no bold detected, try a more lenient approach - just look for the patterns
    for para in doc.paragraphs:
        text = para.text.strip()
        if re.match(r'^\d+\s+years?
...
, text):
            current_date = text
            dates_data[current_date] = []
        # If no bold detected yet, be more lenient - just check pattern
        elif not dates_data and re.match(r'^[A-Z][a-z]+\s+\d+
